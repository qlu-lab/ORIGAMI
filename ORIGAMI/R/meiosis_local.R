...
meiosis <- function(ind_data,cohort,est_ori_file,est_new_file,num_sibling,output_path,
  ...
  meiosis2 <- function(){
      ...
      quantile0 = quantile(map$pos)
      quan0 =c()
      for(i in 1:length(quantile0)){
        quan0 = rbind(quan0,map[which.min(abs(map$pos-quantile0[i])),])
      }
      quan0$cM_new = c(quan0$cM[-1]-quan0$cM[-nrow(quan0)],0)
      pos=c()
      for(j in 1:nrow(quan0)){
        temp = simcross::sim_crossovers(quan0$cM_new[j],m=3)
        if(length(temp)!=0)
          pos = c(pos,temp+quan0$cM[j])
      }
